...
public class ExternalEnvironmentPressure : MonoBehaviour
{
    RawImage icon;
    ...
    void Update()
    {
        ...
        if (TelemetryController.suitSwitch != null)
        {
            ...
            if (TelemetryController.external_environment_pressure_warning)
                DisplayWarning(contrast);
            else if (TelemetryController.external_environment_pressure_critical || switchValue == "1")
    ...
    public void UpdateValue(int contrast_idx)
    {
        ...
        float sub_pressure_float = float.Parse(TelemetryController.telemetryData.p_sub);
        float delta = TelemetryController.p_sub_warning_thr_up;

        value = transform.Find("value_position").transform.GetComponentInChildren<Text>();
        value.text = sub_pressure_float.ToString("0.00") + " psia";
