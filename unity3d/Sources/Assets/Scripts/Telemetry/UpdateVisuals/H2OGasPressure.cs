...
public class H2OGasPressure : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (TelemetryController.suitSwitch != null)
        {
            ...
            if (TelemetryController.H2O_gas_pressure_warning)
                DisplayWarning(contrast);
            else if (TelemetryController.H2O_gas_pressure_critical || switchValue == "1" || switchValueSSPE == "1")
    ...
    public void UpdateValue(int contrast_idx)
    {
        ...
        float h2o_gas_pressure_float = float.Parse(TelemetryController.telemetryData.p_h2o_g);
        float delta = TelemetryController.p_h2o_g_warning_thr_up;

        value = transform.Find("value_position").transform.GetComponentInChildren<Text>();
        value.text = h2o_gas_pressure_float.ToString("0.00") + " psia";
