...
public class H2OLiquidPressure : MonoBehaviour
{
    RawImage icon;
    Text icon_name, value;
    int contrast;
    string switchValue;
    string switchValueSSPE;
    ...
    void Update()
    {
        ...
        if (TelemetryController.suitSwitch != null)
        {
            ...
            switchValueSSPE = TelemetryController.suitSwitch.sspe;

            if (TelemetryController.H2O_liquid_pressure_warning)
                DisplayWarning(contrast);
            else if (TelemetryController.H2O_liquid_pressure_critical || switchValue == "1" || switchValueSSPE == "1")
    ...
    public void UpdateValue(int contrast_idx)
    {
        ...
        float h2o_liquid_pressure_float = float.Parse(TelemetryController.telemetryData.p_h2o_l);
        float delta = TelemetryController.p_h2o_l_warning_thr_up;

        value = transform.Find("value_position").transform.GetComponentInChildren<Text>();
        value.text = h2o_liquid_pressure_float.ToString("0.00") + " psia";
