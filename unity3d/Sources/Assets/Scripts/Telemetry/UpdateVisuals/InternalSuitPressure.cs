...
public class InternalSuitPressure : MonoBehaviour
{
    RawImage icon;
    ...
    void Update()
    {
        ...
        {
            switchValue = TelemetryController.suitSwitch.sspe;
            if (TelemetryController.internal_suit_pressure_warning)
                DisplayWarning(contrast);
            else if (TelemetryController.internal_suit_pressure_critical || switchValue == "1")
    ...
    public void UpdateValue(int contrast_idx)
    {
        ...
        float internal_suit_pressure_float = float.Parse(TelemetryController.telemetryData.p_suit);
        float delta = TelemetryController.p_suit_warning_thr_up;

        value = transform.Find("value_position").transform.GetComponentInChildren<Text>();
        value.text = internal_suit_pressure_float.ToString("0.00") + " psia";
