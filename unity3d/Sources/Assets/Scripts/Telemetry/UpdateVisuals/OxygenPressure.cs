...
{
    RawImage icon;
    Text icon_name, value; int contrast;
    string switchValue;
    ...
    void Update()
    {
        ...
        if (TelemetryController.suitSwitch != null)
        {
            ...
            if (TelemetryController.oxygen_pressure_warning)
                DisplayWarning(contrast);
            else if (TelemetryController.oxygen_pressure_critical || switchValue == "1" || switchValueSSPE == "1")
    ...
    public void UpdateValue(int contrast_idx)
    {
        ...
        float oxygen_pressure_float = float.Parse(TelemetryController.telemetryData.p_o2);
        float delta = TelemetryController.p_o2_warning_thr_up;

        value = transform.Find("value_position").transform.GetComponentInChildren<Text>();
        value.text = oxygen_pressure_float.ToString("0.00") + " psia";
